...
interface TimelineItemProps extends Omit<HTMLMotionProps<"li">, "ref"> {
  ...
  /** Error message */
  error?: string;
  /** Custom React component to render instead of title/description */
...
const TimelineItem = React.forwardRef<HTMLLIElement, TimelineItemProps>(
  (
    ...
    const commonClassName = cn(
      "relative w-full mb-6 md:mb-8 last:mb-0",
      className
    );
    ...
    if (error) {
      return (
        <motion.li
          ...
          {...props}
        >
          <div className="w-full bg-gray-400 grid grid-cols-[minmax(auto,8rem)_auto_1fr] items-start px-4">
            <div className="pr-4 text-right">
              <TimelineTime className="text-destructive w-fit whitespace-nowrap">
                {date}
              </TimelineTime>
            </div>
    ...
    const content = (
      ...
      >
        ...
        <div className="block md:hidden">
          <div className="flex items-start gap-4 mb-3">
            <div className="flex flex-col items-center flex-shrink-0">
              ...
              {showConnector && (
                <div className="flex-1 min-h-[60px] w-0.5 bg-border mt-2" />
              )}
            </div>
            <div className="flex-1 pt-1">
              <TimelineTime className="text-xs font-medium text-muted-foreground mb-2 block">
                {date}
              </TimelineTime>
        ...
        <div className="hidden md:grid md:grid-cols-[minmax(140px,180px)_auto_1fr] md:gap-6 md:items-start">
          {/* Date */}
          <div className="flex flex-col justify-start pt-2">
            <TimelineTime className="text-right text-sm font-medium">
              {date}
            </TimelineTime>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            {showConnector && (
              <div className="flex-1 min-h-[120px] w-0.5 bg-border mt-3" />
            )}
          </div>

          {/* Content */}
          <TimelineContent className="flex-1 w-full">
            {customComponent ? (
              customComponent
            ) : (
              <>
